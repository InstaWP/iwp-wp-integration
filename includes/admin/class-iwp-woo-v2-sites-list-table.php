...
/**
 * IWP_Woo_V2_Sites_List_Table class
 */
class IWP_Woo_V2_Sites_List_Table extends WP_List_Table {
    ...
    public function get_columns() {
        return array(
            ...
            'site_url'    => __('Site URL', 'instawp-integration'),
            'username'    => __('Username', 'instawp-integration'),
            'password'    => __('Password', 'instawp-integration'),
            'user'        => __('User', 'instawp-integration'),
            'source'      => __('Source', 'instawp-integration'),
            'status'      => __('Status', 'instawp-integration'),
            'created'     => __('Created', 'instawp-integration')
    ...
    public function get_bulk_actions() {
        return array(
            'delete' => __('Delete Sites', 'instawp-integration')
        );
    }
    ...
    private function get_sites_from_database() {
        ...
        // Get sites from database table
        $db_sites = IWP_Woo_V2_Sites_Model::get_all(array('limit' => 100));
        ...
        foreach ($db_sites as $db_site) {
            ...
            $site = array(
                ...
                'source'       => ucfirst($db_site->source) . ($order ? ' Order' : ''),
    ...
    public function column_site_url($item) {
        ...
        if (!empty($item['site_id'])) {
            $actions['open_instawp'] = sprintf('<a href="%s" target="_blank">%s</a>', 
                ...
                __('Open in InstaWP', 'instawp-integration')
            );
        }
        ...
        if (!empty($item['site_id'])) {
            ...
            $actions['delete'] = sprintf('<a href="%s" onclick="return confirm(\'%s\')">%s</a>', 
                esc_url($delete_url), 
                esc_js(__('Are you sure you want to delete this site?', 'instawp-integration')),
                __('Delete', 'instawp-integration')
            );
        }
    ...
    public function column_status($item) {
        ...
        switch ($status) {
            case 'completed':
                ...
                $text = __('Active', 'instawp-integration');
                break;
            case 'progress':
                $class = 'iwp-status-progress';
                $text = __('Creating', 'instawp-integration');
                break;
            case 'failed':
                $class = 'iwp-status-failed';
                $text = __('Failed', 'instawp-integration');
                break;
            default:
                $class = 'iwp-status-unknown';
                $text = __('Unknown', 'instawp-integration');
        }
    ...
    public function no_items() {
        esc_html_e('No InstaWP sites found. Sites will appear here after they are created through WooCommerce orders or shortcodes.', 'instawp-integration');
    }
}